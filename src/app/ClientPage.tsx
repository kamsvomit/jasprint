...
export default function ClientPage({ initialProducts, initialActiveTool = null }: ClientPageProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-arsenic/[0.02] selection:bg-red-500/30">
      ...
      {/* Floating buttons — scroll up + WA */}
      {showScrollTop && (
        <div className="fixed bottom-6 right-6 z-[100] flex flex-col items-end gap-2">
          {/* WA floating — hanya di homepage */}
          {isHome && (
            <a
              href={`https://wa.me/${WA_NUMBER}?text=${encodeURIComponent('Halo jasprint! Saya mau konsultasi cetak nih 🙏')}`}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-2 bg-green-500 hover:bg-green-600 text-white font-black text-xs py-3 px-4 rounded-full shadow-2xl transition-all active:scale-95"
            >
          ...
          <button
            ...
            className="w-10 h-10 bg-white dark:bg-arsenic rounded-full shadow-lg flex items-center justify-center text-red-500 hover:text-red-600 hover:shadow-xl transition-all active:scale-95 border border-arsenic/10"
          >
