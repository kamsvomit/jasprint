...
export default function Header({ onSearch, searchQuery, theme, toggleTheme }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-[100] header-apple">
      {/* ── Main bar ── */}
      <div className="w-full max-w-6xl mx-auto px-4 sm:px-8 lg:px-12 h-12 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          {/* Theme */}
          <button onClick={toggleTheme} className="p-1.5 hover:bg-arsenic/5 rounded-md transition-colors" title="Ganti Tema">
            {theme === 'dark'
              ? <Sun className="w-4 h-4" style={{color:"#dc2626", stroke:"currentColor"}} />
              : <Moon className="w-4 h-4" style={{color:"#dc2626", stroke:"currentColor"}} />
            }
          </button>
          ...
          <div className="relative flex items-center">
            ...
            >
              <Search className="w-4 h-4" style={{color:"#dc2626", stroke:"currentColor"}} />
            </button>
          </div>
          ...
          >
            {isMenuOpen
              ? <X className="w-4 h-4" style={{color:"#dc2626", stroke:"currentColor"}} />
              : <Menu className="w-4 h-4" style={{color:"#dc2626", stroke:"currentColor"}} />
            }
          </button>
        </div>
      </div>
      ...
      {/* ── Dropdown menu ── */}
      {isMenuOpen && (
        <div className="absolute top-12 right-0 w-56 header-apple border-t border-arsenic/10 shadow-xl rounded-bl-2xl overflow-hidden">
          <nav className="py-2">
            {navLinks.map((link) => (
              <button
                key={link.href}
